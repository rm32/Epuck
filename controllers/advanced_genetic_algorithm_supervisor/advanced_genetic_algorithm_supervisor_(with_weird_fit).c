...
#define GENOTYPE_SIZE (OUTPUT*INPUT)
#define time 120
  ...
  double dist[1] = {0.0};
   if (wb_receiver_get_queue_length(receiver) > 0) {
    ...
    double sum_sensor_values = 0.0;
     ...
     printf("dist: %f, sensors: %f, cliff: %f | %f \n",(fabs(dist[0])/18000),1/(s[7]/sum_sensor_values + (2 * s[6]/sum_sensor_values) + (5 * s[7]/sum_sensor_values) + (5 * s[0]/sum_sensor_values) + (2 * s[1]/sum_sensor_values) + s[2]/sum_sensor_values),cliff,w[0]-w[1]);
     
    // distance (abs so not negative) * 1/sensor (we want to penalize higher values) * if it fell off
      //fitness =( fabs(200*dist[0]) * 1/sum_sensor_values * cliff )/1000;// * 1/(fabs(w[0]-w[1]));
    // prepare for receiving next genes packet
    fitness = cliff*((fabs(dist[0])/18000) * 1/(s[7]/sum_sensor_values + (2 * s[6]/sum_sensor_values) + (5 * s[7]/sum_sensor_values) + (5 * s[0]/sum_sensor_values) + (2 * s[1]/sum_sensor_values) + s[2]/sum_sensor_values));
